...
qiniu.conf.ACCESS_KEY = 'wVZePLDZ-HfUeK2VowBcuCGJxLpqtb0KW1sMbKqw';
qiniu.conf.SECRET_KEY = 'H-YiKoEAZy_MyEupfgdNbZoVtBaYumx1IBDufoyL';
...
export default class baseComponent {
  ...
  async fetch(url='',data={},type="GET",resType='JSON'){
    ...
    if(type==='GET'){
      ...
      if(dataStr!==''){
        dataStr = dataStr.substr(0,data.lastIndexOf('&'))
        url = url + '?' + dataStr;
      }
    }
    let requestConfig = {
    ...
    return new Promise((resolve, reject) => {
      ...
      form.parse(req,async (err,fileds,files)=>{
        ...
        try{
          ...
          const token = this.uptoken('node-eleme',key);
